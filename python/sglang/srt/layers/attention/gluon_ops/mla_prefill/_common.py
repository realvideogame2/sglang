...
def _buffer_load_to_shared_cast_safe(dst_smem, src_base, offsets, mask, other):
    ...
    When smem and source element types match we take the direct gmem->LDS
    DMA path (``amdg.buffer_load_to_local``). Only mixed-dtype loads
    (e.g. fp8 source into bf16 smem) need the register-through fallback.

    NOTE: ``src_base.dtype`` is a ``pointer_type`` (``ptr<T>``); the actual
    element type is ``src_base.dtype.element_ty``. Comparing against the
    pointer type itself will ALWAYS be false and silently disable the DMA
